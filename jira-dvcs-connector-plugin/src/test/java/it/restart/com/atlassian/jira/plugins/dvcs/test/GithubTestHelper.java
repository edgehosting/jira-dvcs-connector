...
{
    public static List<String> getHookUrls(String gitUrl, String project, String username, String password)
    {
        final GitHubRESTClientImpl restClient = new GitHubRESTClientImpl();
        final Repository repository = new Repository();
        repository.setOrgHostUrl(gitUrl);
        repository.setOrgName(username);
        repository.setSlug(project);
        final Credential credential = new Credential();
        credential.setAccessToken("bogus");
        repository.setCredential(credential);
        List<GitHubRepositoryHook> hooks = restClient.getHooks(repository, username, password);
