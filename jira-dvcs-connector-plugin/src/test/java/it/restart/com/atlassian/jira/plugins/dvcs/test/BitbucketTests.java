...
public class BitbucketTests extends DvcsWebDriverTestCase implements BasicTests, ActivityStreamsTest
{
    ...
    public void beforeClass()
    {
        ...
        // log in to Bitbucket
        new MagicVisitor(jira).visit(BitbucketLoginPage.class).doLogin(FIRST_ACCOUNT, PasswordUtil.getPassword(FIRST_ACCOUNT));
        // setup up OAuth from bitbucket
        oAuth = new MagicVisitor(jira).visit(BitbucketOAuthPage.class, FIRST_ACCOUNT).addConsumer();
    ...
    public void afterClass()
    {
        ...
        new MagicVisitor(jira).visit(BitbucketOAuthPage.class, FIRST_ACCOUNT).removeConsumer(oAuth.applicationId);
    ...
    {
        testPostCommitHookAddedAndRemoved(FIRST_ACCOUNT, AccountType.BITBUCKET, "public-hg-repo", jira, getOAuthCredentials());
    }
    ...
    {
        addOrganization(AccountType.BITBUCKET, SECOND_ACCOUNT, getOAuthCredentials(), false);

        AccountsPageAccountRepository repository = enableRepository(SECOND_ACCOUNT, "testemptyrepo", true);
    ...
    {
        addOrganization(AccountType.BITBUCKET, SECOND_ACCOUNT, getOAuthCredentials(), true);

        AccountsPage accountsPage = jira.visit(AccountsPage.class);
        AccountsPageAccount account = accountsPage.getAccount(AccountsPageAccount.AccountType.BITBUCKET, SECOND_ACCOUNT);
