 ...
 */
public class MissingCommitsGithubEnterpriseTest extends BitBucketBaseOrgTest
{
    private static final String GITHUB_URL = "http://192.168.2.47";
    ...
    {
        GitHubClient gitHubClient = GithubOAuthUtils.createClient(GITHUB_URL);
...
//        jira.getTester().gotoUrl(GithubLoginPage.PAGE_URL);
        jira.getTester().gotoUrl(GITHUB_URL+"/login");
...
//        jira.getTester().gotoUrl(GithubRegisterOAuthAppPage.PAGE_URL);
        jira.getTester().gotoUrl(GITHUB_URL+"/settings/applications/new");
    ...
    private void pushGithubRepository(String pathToRepoZip) throws IOException, URISyntaxException, InterruptedException
    {
        ...
        String gitPushUrl = getPushUrl(GITHUB_URL, GITHUB_REPO_OWNER, GITHUB_REPO_PASSWORD, MISSING_COMMITS_REPOSITORY_NAME);
    ...
    private String getPushUrl(String githubUrl, String owner, String password, String repositoryName) throws MalformedURLException
    {
        URL url = new URL(githubUrl);
        
        return String.format("%1$s://%2$s:%3$s@%4$s/%2$s/%5$s.git", url.getProtocol(), owner,
                password, url.getHost(), repositoryName);
    }
