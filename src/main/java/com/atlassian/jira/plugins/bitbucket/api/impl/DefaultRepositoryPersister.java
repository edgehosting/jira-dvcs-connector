...
public class DefaultRepositoryPersister implements RepositoryPersister
{
        ...
        String username, String password, String adminUsername, String adminPassword, String accessToken)
    {
        ...
        {
            map.put(ProjectMapping.USERNAME, username);
            map.put(ProjectMapping.PASSWORD, password);
        }
    ...
    public IssueMapping getIssueMapping(final String node)
    {
        return activeObjects.executeInTransaction(new TransactionCallback<IssueMapping>()
        {
            ...
            {
                IssueMapping[] mappings = activeObjects.find(IssueMapping.class, IssueMapping.NODE + " = ?", node);
